...
int nrf24l01_connect_wait(int8_t cli_sockfd)
{
	...
	struct nrf14_ll_crtl_pdu *ipdu;
	struct nrf24_ll_version_ind *ipdu_pay;

	ipdu = (struct nrf14_ll_crtl_pdu *)datagram;
	ipdu_pay = (struct nrf24_ll_version_ind *)ipdu->payload;
...
int nrf24L01_connect_response(uint8_t sockfd)
{
	...
	uint8_t datagram[NRF24_MTU];
	struct nrf14_ll_crtl_pdu *opdu;
	struct nrf24_ll_version_ind *payload;
	int err;
	size_t len;
	size_t offset;

	opdu = (struct nrf14_ll_crtl_pdu *)datagram;
	/* send version pkt */
	payload = (struct nrf24_ll_version_ind *) opdu->payload;
...
static int nrf24L01_accept(int sockfd)
{
	...
	ipdu = (struct nrf24_ll_mgmt_pdu *) datagram;
	payload = (struct nrf24_ll_mgmt_connect *) ipdu->payload;
