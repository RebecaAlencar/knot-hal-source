							...
							gpointer user_data)
{
	...
	GIOCondition watch_cond = G_IO_HUP | G_IO_NVAL | G_IO_ERR | G_IO_IN;
	...
	/* Forward using unix socket to knotd */
	if (!session->knotd_io) {
		printf("Not connected to knotd, trying to reconnect\n\r");
		/* Reconnect to knotd */
		...
		printf("Already connected to knotd, getting socket (%d)\n\r",
								 knotdfd);
	}
